...
CLOUD_TARGETS:=bin/${MICROSERVICE}_ll3 bin/${MICROSERVICE}_fe3 bin/${MICROSERVICE}_fe

TOOL_TARGETS:=bin/gen_doc

KC_COMMON_JAR_VERSION:=$(shell bin/extract_kc_common_version_from_pom.sh ${MICROSERVICE}_dl3/pom.xml)
...
.PHONY: ${MICROSERVICE}_dl3 ${MICROSERVICE}_ll3 ${MICROSERVICE}_fe3 ${MICROSERVICE}_fe utils
...
githook: .git/hooks/pre-commit version
...
${MICROSERVICE}_fe: githook bin/${MICROSERVICE}_fe

${MICROSERVICE}_ll3: githook bin/${MICROSERVICE}_ll3

${MICROSERVICE}_dl3: githook ${KC_COMMON_JAR} ${MICROSERVICE}_dl3/pom.xml
	sh -c 'cd ${MICROSERVICE}_dl3; make'

${MICROSERVICE}_ll3/version/version.go: .version
	sh -c 'mkdir -p ${MICROSERVICE}_ll3/version && ./bin/gen_version_go.sh .version go.mod >$@'

${MICROSERVICE}_fe3/version/version.go: .version
	sh -c 'mkdir -p ${MICROSERVICE}_fe3/version && ./bin/gen_version_go.sh .version go.mod >$@'

${MICROSERVICE}_fe/version/version.go: .version
	sh -c 'mkdir -p ${MICROSERVICE}_fe/version && ./bin/gen_version_go.sh .version go.mod >$@'
...
bin/${MICROSERVICE}_fe: ${MICROSERVICE}_fe/version/version.go
	@echo "========== Compiling $@ =========="
	sh -c '$(GO) build $(GOFLAGS) -o $@ git.kaiostech.com/cloud/${MICROSERVICE}/${MICROSERVICE}_fe'

utils/version/version.go: .version
	sh -c 'mkdir -p utils/version && ./bin/gen_version_go.sh .version go.mod >$@'

version: utils/version/version.go

${KC_COMMON_JAR}: ${MICROSERVICE}_dl3/pom.xml
	sh -c "./bin/gen_kc_common.sh ${KC_COMMON_JAR_VERSION}"

bin/gen_doc: utils/gen_doc/gen_doc.go utils/version/version.go
	@echo "========== Compiling $@ =========="
...
deploy: clean ${MICROSERVICE}_dl3 ${CLOUD_TARGETS} ${KC_COMMON_JAR}
	version=`cat .version`; output_folder=${MICROSERVICE}-$${version}; mkdir -p /tmp/$${output_folder}/bin && cp -a bin/{${MICROSERVICE}_fe3,${MICROSERVICE}_fe,${MICROSERVICE}_ll3,run_${MICROSERVICE}_dl3.sh,run3.sh,service_functions.sh,start_service.sh,stop_service.sh,${MICROSERVICE}_dl3-$${version}.jar,functions.sh,jars,conf} /tmp/$${output_folder}/bin/ && cp ${KC_COMMON_JAR} /tmp/$${output_folder}/bin/jars && mkdir -p /tmp/$${output_folder}/conf/ && cp -a RELEASE.txt /tmp/$${output_folder} && cp -a conf/{${MICROSERVICE}.conf.dev,${MICROSERVICE}.conf.local,${MICROSERVICE}.conf.k5,jwt_key.priv,jwt_key.pub,${MICROSERVICE}_fe3_log.xml,${MICROSERVICE}_ll3_log.xml} /tmp/$${output_folder}/conf/ && tar -jcvf /tmp/${MICROSERVICE}-$${version}-${GOARCH}.tar.bz2 -C /tmp ${MICROSERVICE}-$${version} && mv /tmp/${MICROSERVICE}-$${version}-${GOARCH}.tar.bz2 . && rm -Rf /tmp/${MICROSERVICE}-$${version}
...
clean:
	...
	@echo "Deleting generated version files ..."; for version_dir in ${MICROSERVICE}_ll3/version ${MICROSERVICE}_fe3/version ${MICROSERVICE}_fe/version utils/version; do if [ -d "$${version_dir}" ]; then rm -Rf $${version_dir} && echo $${version_dir}; fi; done
...
clear_cache:
	...
	@sh -c 'cd ${MICROSERVICE}_dl3 && make clear_cache'
